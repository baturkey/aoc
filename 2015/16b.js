/* 405 */
const input = `Sue 1: goldfish: 6, trees: 9, akitas: 0
...
function process(s) {
    const [_, ...components] = s.split(': ');
    return components.join(': ').split(', ').reduce((acc, cur) => {
        const [name, qty] = cur.split(': ');
        acc[name] = qty;
        return acc;
    }, {});
}
...
for (let i = 0; i < sues.length; i++) {
    let valid = true;
    for (let k of Object.keys(sues[i])) {
        if (['cats', 'trees'].includes(k)) {
            if (sues[i][k] <= target[k]) {
                valid = false;
                break;
            }
