...
for (let i = 0; i < SIZE; i++) {
    lights[i] = Array(SIZE).fill(false);
}

function process(s) {
    if (s.substring(0, 6) == 'toggle') {
        toggle(extract(s.substring(7)));
    } else if (s.substring(0, 7) == 'turn on') {
        on(extract(s.substring(8)));
    } else if (s.substring(0, 8) == 'turn off') {
        off(extract(s.substring(9)));
    } else {
        console.log('Illegal input: ' + s);
    }
}

function extract(s) {
    return s.split(' through ').flatMap(x => x.split(',')).map(a => parseInt(a));
}

function on(a) {
    for (let x = a[0]; x <= a[2]; x++) {
        for (let y = a[1]; y <= a[3]; y++) {
            lights[x][y] = true;
        }
    }
}
