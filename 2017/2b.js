...
let foo = input
    .split('\n')
//    .map(a => analyzer(a.split('\t').map(a => parseInt(a, 10)).sort((a, b) => b - a)))
    .map(a => fx(a.split('\t').map(a => parseInt(a, 10)).sort((a, b) => b - a)))
    .reduce((a, b) => a + b);


console.log(foo);

function analyzer(sorted) {
...
function fx(sorted) {
    ...
    function bar(x, y) {
        ...
        return bar(x, y.slice(1));
    }

    const result = bar(head, rest);
    if (result == 0) {
        return fx(rest);
    }
    return result;
