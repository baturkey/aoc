...
function execute(pos) {
    ...
    while(1) {
        ...
        function param(offset) {
            ...
            return typeof pos[loc] == 'undefined' ? 0 : +pos[loc];
        }
        ...
        if (1 == opcode) {
            ...
            pos[loc(1)] = commands[commandcounter++];
            ip += 2;
        } else if (4 == opcode) {
            const o = param(1);
            if (o == 10) {
                ...
                output[output.length - 1].push(param(1));
            }
            ip += 2;
        } else if (5 == opcode) {
...
const asciioutput = output.filter(a => a.length).map(x => x.map(a => String.fromCodePoint(a)));
const height = asciioutput.length;
const width = asciioutput[0].length;
let x, y;

for (let h = 0; h < height; h++) {
    for (let w = 0; w < width; w++) {
        if (['^', 'v', '<', '>'].includes(asciioutput[h][w])) {
            [y, x] = [h, w];
        }
    }
}

function probe(a, x, y, olddir) {
    const available = [];
    ...
    return available.filter(a => a != opposite[olddir])[0];
}

function findEnd(a, x, y, dir) {
    let counter = 0;
    if (dir == 'N') {
...
//     direction = probe(asciioutput, x, y, direction);
//     [x, y, result] = findEnd(asciioutput, x, y, direction);
//     console.log(lookup[olddirection][direction], result);
// } 
...
// L, 4, L, 4, L, 6, R, 10, L, 6, L, 4, L, 4, L, 6, R, 10, L, 6, L, 12, L, 6, R, 10
