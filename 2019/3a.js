...
console.log(
    intersections(
        test.split('\n').map(s => segmentize(s.split(',')))
    ).reduce((acc, cur) => Math.min(Math.abs(cur[0]) + Math.abs(cur[1]), acc), Number.MAX_SAFE_INTEGER)
);
...
function segmentize(a) {
    ...
    for (let i = 0; i < a.length; i++) {
        ...
        if (direction == 'U') {
            output.vertical.push({x, start: y, end: y + distance});
            y += distance;
        } else if (direction == 'R') {
            output.horizontal.push({y, start: x, end: x + distance});
            x += distance;
        } else if (direction == 'D') {
            output.vertical.push({x, start: y - distance, end: y});
            y -= distance;
        } else if (direction == 'L') {
            output.horizontal.push({y, start: x - distance, end: x});
            x -= distance;
        }
    }
