...
const sendCommand = function() {
    ...
    return input => {
        ...
        while(1) {
            ...
            const loc = offset => parseInt(code[ip + offset]) + (m[offset] == 2 ? relbase : 0);

            function param(offset) {
                const loc = m[offset] == 1 ? ip + offset : (parseInt(code[ip + offset]) + (m[offset] == 2 ? relbase : 0));
                return parseInt(typeof code[loc] == 'undefined' ? 0 : code[loc]);
            }

            if (1 == opcode) {
                code[loc(3)] = '' + (param(1) + param(2));
                ip += 4;
            } else if (2 == opcode) {
                code[loc(3)] = '' + param(1) * param(2);
                ip += 4;
                ...
                code[loc(3)] = param(1) < param(2) ? 1 : 0;
                ip += 4;
            } else if (8 == opcode) {
                code[loc(3)] = param(1) == param(2) ? 1 : 0;
                ip += 4;
...
const opposite = {1: 2, 2: 1, 3: 4, 4: 3};
const map = [];
const comefrom = [];
for (let x = 0; x <= 40; x++) {
    map[x] = [];
...
while(work.length) {
    ...
    while (current.x != x || current.y !== y) {
        [x, y] = move(map, x, y, comefrom[x][y].direction);
    }
    ...
    while (true) {
        ...
        if (available.length > 1) {
            work.push({x, y, distance});
        }

        let [newX, newY, s] = move(map, x, y, available[0]);
        distance++;

        comefrom[newX][newY] = {x, y, direction: opposite[available[0]]};
        [x, y] = [newX, newY];
        map[x][y] = '.';
    }
}
