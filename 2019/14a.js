...
function timesToRun(target, amount) {
    if (target < amount) {
        return 1;
    }

    return Math.floor(target / amount) + (target % amount ? 1 : 0);
}

function lookup(chemical, quantity) {
    ...
    for (let input of inputChemicals) {
        if (input == 'ORE') {
            ...
            if (needed > available[input]) {
                ...
                const timestorun = timesToRun(needtoproduce, reactions[input].quantity);
                total += lookup(input, needtoproduce);
                available[input] += timestorun * reactions[input].quantity - needed;
            } else {
