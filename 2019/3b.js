...
const directions = test.split('\n').map(s => s.split(','));
const segments = directions.map(segmentize);

console.log(directions);
console.log(segments[0]);
console.log(segments[1]);
//console.log(intersections(segments));
console.log(intersections(segments).reduce((acc, cur) => Math.min(cur[2], acc), Number.MAX_SAFE_INTEGER));
...
function segmentize(a) {
    ...
    for (let i = 0; i < a.length; i++) {
        let direction = a[i][0];
        let distance = parseInt(a[i].substring(1));

        if (direction == 'U') {
            output.vertical.push({x, start: y, end: y + distance, steps, direction});
            y += distance;
        } else if (direction == 'R') {
            output.horizontal.push({y, start: x, end: x + distance, steps, direction});
            x += distance;
        } else if (direction == 'D') {
            output.vertical.push({x, start: y - distance, end: y, steps, direction});
            y -= distance;
        } else if (direction == 'L') {
            output.horizontal.push({y, start: x - distance, end: x, steps, direction});
            x -= distance;
        } else {
...
function intersections(a) {
    let output = [];

    function findTotal(h, v) {
        let total = h.steps + v.steps;
    ...
    for (let i = 0; i < a[0].vertical.length; i++) {
        for (let j = 0; j < a[1].horizontal.length; j++) {
            let v = a[0].vertical[i];
            let h = a[1].horizontal[j];
            if (h.start <= v.x && v.x <= h.end && v.start <= h.y && h.y <= v.end) {
                output.push([v.x, h.y, findTotal(h, v)]);
            }
        }
    }

    for (let i = 0; i < a[0].horizontal.length; i++) {
        for (let j = 0; j < a[1].vertical.length; j++) {
            let h = a[0].horizontal[i];
            let v = a[1].vertical[j];
            if (h.start <= v.x && v.x <= h.end && v.start <= h.y && h.y <= v.end) {
                output.push([v.x, h.y, findTotal(h, v)]);
            }
        }
    ...
    return output.filter(a => a[0] + a[1] != 0);
}
