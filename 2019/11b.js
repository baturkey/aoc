...
function execute(pos) {
    ...
    for (let k of Object.keys(map).map(a => parseInt(a)).sort((a, b) => b - a)) {
        map[k]['0'] = 0;
        let output = '';
        Object.keys(map[k]).map(a => parseInt(a)).sort((a, b) => a - b).forEach(a => output += map[k][a] ? '0' : ' ');
        console.log(output);
    }
}
